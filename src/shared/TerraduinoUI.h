...
#undef TERRA_UI_ENABLE_DEBUG_MENU
#endif
#endif // /ifdef NDEBUG

#include <Terraduino.h>
#ifdef TERRA_USE_GUI
#ifndef TerraBaseUI_H
#define TerraBaseUI_H
...
#include "TerraDisplayDrivers.h"
#include "TerraInputDrivers.h"
#include "TerraRemoteControls.h"
#include "TerraMenus.h"
#include "TerraOverviews.h"
...
class TerraduinoBaseUI : public TerraUIInterface, public CustomDrawing {
public:
    // Base UI constructor.
    TerraduinoBaseUI(String deviceUUID,                                     // Device UUID hex string for remote controllability
    ...
    virtual TerraUIData *init(TerraUIData *uiData = nullptr) override;      // UIData instance
    ...
    void init(uint8_t updatesPerSec,                                        // Updates per second (1 to 10)
              Terra_DisplayTheme displayTheme,                              // Display theme to apply
    ...
    inline int getVRAMBufferRows() const { return _isBufferedVRAM && (_display && _display->getGraphicsRenderer()) ? TERRA_UI_SPRITE_MAXYSIZE : 0; }
    ...
    // UIData accessor
    inline TerraUIData *getUIData() { return _uiData; }
    // Input driver accessor
    inline TerraInputDriver *getInput() { return _input; }
    // Display driver accessor
    inline TerraDisplayDriver *getDisplay() { return _display; }
    // Remote server accessor
    inline TcMenuRemoteServer *getRemoteServer() { return _remoteServer; }
    // Overview screen accessor
    inline TerraOverview *getOverview() { return _overview; }
    // Home menu accessor
    inline TerraHomeMenu *getHomeMenu() { return _homeMenu; }
...
protected:
    ...
    TerraUIData *_uiData;                                   // Terra UI data (strong)
    TerraInputDriver *_input;                               // Input driver (owned)
    TerraDisplayDriver *_display;                           // Display driver (owned)
    TcMenuRemoteServer *_remoteServer;                      // Remote control server (owned)
    Vector<TerraRemoteControl *, TERRA_UI_REMOTECONTROLS_MAXSIZE> _remotes; // Remote controls list (owned)
    TerraPin *_backlight;                                   // Backlight control (owned)
    time_t _blTimeout;                                      // Backlight timeout (UTC)
    TerraOverview *_overview;                               // Overview screen (owned)
    TerraHomeMenu *_homeMenu;                               // Home menu screen (owned)
...
#include "TerraDisplayDrivers.hpp"
#include "TerraOverviews.hpp"

#endif // /ifndef TerraBaseUI_H
#endif
