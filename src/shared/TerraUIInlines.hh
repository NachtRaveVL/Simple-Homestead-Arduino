/*  Terraduino: Simple automation controller for homestead resource and environmental systems.
    ...
    Terraduino UI Common Inlines
*/

#include <Terraduino.h>
#ifdef TERRA_USE_GUI
#ifndef TerraUIInlines_HPP
#define TerraUIInlines_HPP
...
inline TerraduinoBaseUI *getBaseUI() { return reinterpret_cast<TerraduinoBaseUI *>(getUI()); }

// Returns the first theme in parameter list that isn't undefined, allowing defaulting chains to be nicely defined.
inline Terra_DisplayTheme definedThemeElse(Terra_DisplayTheme theme1, Terra_DisplayTheme theme2) {
    return theme1 != Terra_DisplayTheme_Undefined ? theme1 : theme2;
}


// LCD Display Setup
struct LCDDisplaySetup {
    Terra_BacklightMode ledMode;        // LCD backlight pin mode (default: Terra_BacklightMode_Normal)
    bool isDFRobotShield;               // Using DF robot shield

    inline LCDDisplaySetup(Terra_BacklightMode ledModeIn = Terra_BacklightMode_Normal, bool isDFRobotShieldIn = false) : ledMode(ledModeIn), isDFRobotShield(isDFRobotShieldIn) { ; }

    static inline LCDDisplaySetup usingDFRobotShield() { return LCDDisplaySetup(Terra_BacklightMode_Normal, true); }
};

// Standard Pixel Display Setup (U8g2OLED & AdaGfx/AdaTFT)
struct PixelDisplaySetup {
    Terra_DisplayRotation rotation;     // Display orientation/rotation (default: R0)
    ...
    Terra_BacklightMode ledMode;        // Backlight/LED/BL pin mode (default: Terra_BacklightMode_Normal)
...
#endif
    Terra_ST77XXKind st77Kind;          // ST7735 tag color or ST7789 screen resolution (default: undef/-1), if ST77XX

    inline PixelDisplaySetup(Terra_DisplayRotation rotationIn = Terra_DisplayRotation_R0, pintype_t dcPinIn = -1, pintype_t resetPinIn = -1, pintype_t ledPinIn = -1, Terra_BacklightMode ledModeIn = Terra_BacklightMode_Normal, uint8_t ledBitResIn = DAC_RESOLUTION,
#ifdef ESP32
...
#endif
                             Terra_ST77XXKind st77KindIn = Terra_ST77XXKind_Undefined)
...
struct TFTDisplaySetup {
    ...
    float ledFrequency;                 // Backlight PWM output frequency, if PWM/ESP
#endif
    Terra_ST77XXKind st77Kind;          // ST7735 tag color (default: undef/-1), if ST7735 (ST7789 enums not used)

    inline TFTDisplaySetup(Terra_DisplayRotation rotationIn = Terra_DisplayRotation_R0, pintype_t ledPinIn = -1, Terra_BacklightMode ledModeIn = Terra_BacklightMode_Normal, uint8_t ledBitResIn = DAC_RESOLUTION,
#ifdef ESP32
                           uint8_t ledChannelIn = 1,
#endif
#ifdef ESP_PLATFORM
...
struct UIDisplaySetup {
    ...
    inline Terra_DisplayRotation getDisplayRotation() const { return dispCfgType == Pixel ? dispCfgAs.gfx.rotation : dispCfgType == TFT ? dispCfgAs.tft.rotation : Terra_DisplayRotation_R0; }
    inline pintype_t getBacklightPin() const { return dispCfgType == LCD ? (pintype_t)(dispCfgAs.lcd.isDFRobotShield ? 10 : 3) : dispCfgType == Pixel ? dispCfgAs.gfx.ledPin : dispCfgType == TFT ? dispCfgAs.tft.ledPin : tpin_none; }
    inline Terra_BacklightMode getBacklightMode() const { return dispCfgType == LCD ? dispCfgAs.lcd.ledMode : dispCfgType == Pixel ? dispCfgAs.gfx.ledMode : dispCfgType == TFT ? dispCfgAs.tft.ledMode : Terra_BacklightMode_Normal; }
...
struct RotaryControlSetup {
    Terra_EncoderSpeed encoderSpeed;    // Encoder cycling speed (detent freq)

    inline RotaryControlSetup(Terra_EncoderSpeed encoderSpeedIn = Terra_EncoderSpeed_HalfCycle) : encoderSpeed(encoderSpeedIn) { ; }
};
...
struct ButtonsControlSetup {
    ...
    inline ButtonsControlSetup(uint8_t repeatSpeedIn = TERRA_UI_KEYREPEAT_SPEED, bool isDFRobotShieldIn = false) : repeatSpeed(repeatSpeedIn), isDFRobotShield(isDFRobotShieldIn) { ; }

    static inline ButtonsControlSetup usingDFRobotShield() { return ButtonsControlSetup(TERRA_UI_KEYREPEAT_SPEED, true); }
};
...
struct ESP32TouchControlSetup {
    ...
    Terra_ESP32Touch_HighRef highVoltage;       // High reference voltage (default: V_2V7)
    Terra_ESP32Touch_LowRef lowVoltage;         // Low reference voltage (default: V_0V5)
    Terra_ESP32Touch_HighRefAtten attenuation;  // High reference voltage attention (default: V_1V)

    inline ESP32TouchControlSetup(uint8_t repeatSpeedIn = TERRA_UI_KEYREPEAT_SPEED, uint16_t switchThresholdIn = 800, Terra_ESP32Touch_HighRef highVoltageIn = Terra_ESP32Touch_HighRef_V_2V7, Terra_ESP32Touch_LowRef lowVoltageIn = Terra_ESP32Touch_LowRef_V_0V5, Terra_ESP32Touch_HighRefAtten attenuationIn = Terra_ESP32Touch_HighRefAtten_V_1V) : repeatSpeed(repeatSpeedIn), switchThreshold(switchThresholdIn), highVoltage(highVoltageIn), lowVoltage(lowVoltageIn), attenuation(attenuationIn) { ; }
};
...
struct MatrixControlSetup {
    ...
    Terra_EncoderSpeed encoderSpeed;    // Encoder cycling speed (optional)

    inline MatrixControlSetup(millis_t repeatDelayIn = 850, millis_t repeatIntervalIn = 350, Terra_EncoderSpeed encoderSpeedIn = Terra_EncoderSpeed_HalfCycle) : repeatDelay(repeatDelayIn), repeatInterval(repeatIntervalIn), encoderSpeed(encoderSpeedIn) { ; }
};

// Touchscreen Input Setup
struct TouchscreenSetup {
    Terra_TouchscreenOrientation orient; // Touchscreen orientation tuning (default: Same)
    #ifdef TERRA_UI_ENABLE_XPT2046TS
        SPIClass *spiClass;
    #endif

    #ifndef TERRA_UI_ENABLE_XPT2046TS
        inline TouchscreenSetup(Terra_TouchscreenOrientation orientIn = Terra_TouchscreenOrientation_Same) : orient(orientIn) { ; }
    #else
        inline TouchscreenSetup(Terra_TouchscreenOrientation orientIn = Terra_TouchscreenOrientation_Same, SPIClass *spiClassIn = TERRA_USE_SPI) : orient(orientIn), spiClass(spiClassIn) { ; }
    #endif
};
...
#endif // /ifndef TerraUIInlines_HPP
#endif
