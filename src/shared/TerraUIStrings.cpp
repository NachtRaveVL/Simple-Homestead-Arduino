...
{
    static TerraUI_String _lookupStrNum = (TerraUI_String)-1; // Simple LRU cache reduces a lot of lookup access
    ...
    if (_uiStrDataAddress != (uint16_t)-1) {
        ...
        if (eeprom) {
            ...
            {   String retVal;
                char buffer[TERRA_STRING_BUFFER_SIZE] = {0};
                uint16_t bytesRead = eeprom->readBlock(lookupOffset, (uint8_t *)&buffer[0], TERRA_STRING_BUFFER_SIZE);
                retVal.concat(charsToString(buffer, bytesRead));

                while (strnlen(buffer, TERRA_STRING_BUFFER_SIZE) == TERRA_STRING_BUFFER_SIZE) {
                    lookupOffset += TERRA_STRING_BUFFER_SIZE;
                    bytesRead = eeprom->readBlock(lookupOffset, (uint8_t *)&buffer[0], TERRA_STRING_BUFFER_SIZE);
    ...
    if (_uiStrDataFilePrefix.length()) {
        ...
        if (sd) {
            ...
            if (file) {
                ...
                {   char buffer[TERRA_STRING_BUFFER_SIZE];
                    file.seek(lookupOffset);
                    auto bytesRead = file.readBytesUntil('\000', buffer, TERRA_STRING_BUFFER_SIZE);
                    retVal.concat(charsToString(buffer, bytesRead));

                    while (strnlen(buffer, TERRA_STRING_BUFFER_SIZE) == TERRA_STRING_BUFFER_SIZE) {
                        bytesRead = file.readBytesUntil('\000', buffer, TERRA_STRING_BUFFER_SIZE);
                        if (bytesRead) { retVal.concat(charsToString(buffer, bytesRead)); }
                    }
                }
...
const char *pgmAddrForStr(TerraUI_String strNum)
{
    switch(strNum) {
        ...
        case TUIStr_Item_TriggerLowMem: {
        ...
        case TUIStr_Item_TriggerSDCleanup: {
        ...
        case TUIStr_Item_TriggerSigTime: {
        ...
        case TUIStr_Item_TriggerSigLocation: {
