...
inline pintype_t pinNumberForPinChannel(int8_t pinChannel) { return isValidChannel(pinChannel) && pinChannel >= 0 ? (pintype_t)(tpin_virtual + pinChannel) : tpin_none; }
// Returns expanded pin channel # [0,16) to use for a given virtual pin # (100+), else pin # [0,99] if not virtual (<100), else -1/invalid
inline uint8_t pinChannelOrPinNumber(pintype_t pinNumber) { return isValidPin(pinNumber) ? (pinNumber >= tpin_virtual ? (pinNumber - tpin_virtual) % 16 : pinNumber) : (uint8_t)-1; }
// Returns expander position # [0,Max] to use for a given virtual pin # (100+), else -1/invalid if not virtual (<100)
inline tposi_t expanderPosForPinNumber(pintype_t pinNumber) { return isValidPin(pinNumber) && pinNumber >= tpin_virtual ? (pinNumber - tpin_virtual) / 16 : (tposi_t)-1; }
// Returns expander position # [0,Max] to use for a given expanded pin channel # [0,16*Max), else -1/invalid if not expanded
inline tposi_t expanderPosForPinChannel(int8_t pinChannel) { return isValidChannel(pinChannel) && pinChannel >= 0 ? pinChannel / 16 : (tposi_t)-1; }
...
inline int8_t pinChannelForMuxerChannel(uint8_t muxChannel) { return muxChannel != (uint8_t)-1 && muxChannel != (uint8_t)tpinchnl_none ? -(int8_t)(1 + constrain(muxChannel,0,125)) : tpinchnl_none; }
// Returns expanded pin channel # [+0,16*Max) to use for a given expander channel # [0,16*Max), else -127/none
inline int8_t pinChannelForExpanderChannel(uint8_t expChannel) { return expChannel != (uint8_t)-1 && expChannel != (uint8_t)tpinchnl_none ? (int8_t)constrain(expChannel,0,127) : tpinchnl_none; }
...
struct TerraPin {
    ...
    Terra_PinMode mode;                                     // Pin mode setting
    int8_t channel;                                         // Pin muxer(-vals)/expander(+vals) channel #, else -127/none

    TerraPin();
    TerraPin(int classType, pintype_t pinNumber = tpin_none, Terra_PinMode pinMode = Terra_PinMode_Undefined, int8_t pinChannel = tpinchnl_none);
    TerraPin(const TerraPinData *dataIn);

    explicit operator TerraDigitalPin() const;
    explicit operator TerraAnalogPin() const;
    ...
    inline bool isValid() const { return isValidPin(pin) && mode != Terra_PinMode_Undefined; }
    inline bool isVirtual() const { return isValidPin(pin) && pin >= tpin_virtual; }
    ...
    inline bool isOutput() const { return mode == Terra_PinMode_Digital_Output ||
                                          mode == Terra_PinMode_Digital_Output_PushPull ||
...
// Digital Pin
struct TerraDigitalPin : public TerraPin, public TerraDigitalInputPinInterface, public TerraDigitalOutputPinInterface {
    bool activeLow;                                         // Active-low trigger state boolean

    TerraDigitalPin();
    TerraDigitalPin(pintype_t pinNumber,                    // Digital pin number (e.g. D0, D1)
                    ard_pinmode_t pinMode,                  // Arduino pin mode (e.g. INPUT, OUTPUT, determines activeLow trigger state)
                    int8_t pinChannel = tpinchnl_none);     // Pin channel select, else -127/none
    TerraDigitalPin(pintype_t pinNumber,                    // Digital pin number (e.g. D0, D1)
                    Terra_PinMode pinMode,                  // Terraduino pin mode (determines activeLow trigger state)
                    int8_t pinChannel = tpinchnl_none);     // Pin muxer/expander channel #, else -127/none
    TerraDigitalPin(pintype_t pinNumber,                    // Digital pin number (e.g. D0, D1)
                    ard_pinmode_t pinMode,                  // Arduino pin mode (e.g. INPUT, OUTPUT)
                    bool isActiveLow,                       // Explicit pin active-low trigger state boolean
                    int8_t pinChannel = tpinchnl_none);     // Pin muxer/expander channel #, else -127/none
    TerraDigitalPin(pintype_t pinNumber,                    // Digital pin number (e.g. D0, D1)
                    Terra_PinMode pinMode,                  // Terraduino pin mode
                    bool isActiveLow,                       // Explicit pin active-low trigger state boolean
                    int8_t pinChannel = tpinchnl_none);     // Pin muxer/expander channel #, else -127/none
...
#ifndef ARDUINO
    inline void setSimulatedState(bool active) { _lastState = activeLow ? (active ? LOW : HIGH) : (active ? HIGH : LOW); }
#endif

protected:
#ifndef ARDUINO
    ard_pinstatus_t _lastState;                             // Last simulated electrical pin state
#endif
};

// Analog Pin
struct TerraAnalogPin : public TerraPin, public TerraAnalogInputPinInterface, public TerraAnalogOutputPinInterface {
    ...
    TerraAnalogPin();
    TerraAnalogPin(pintype_t pinNumber,                     // Analog pin number (e.g. A0, A1)
                   ard_pinmode_t pinMode,                   // Arduino pin mode (e.g. INPUT, OUTPUT)
...
protected:
#ifndef ARDUINO
    int _lastValue;                                         // Last simulated raw analog value
#endif
};
...
public:
    TerraPinMuxer();
    TerraPinMuxer(TerraPin signalPin,
                  pintype_t *muxChannelPins, int8_t muxChannelBits,
                  TerraDigitalPin chipEnablePin = TerraDigitalPin(),
                  TerraDigitalPin interruptPin = TerraDigitalPin());
    ...
    inline const TerraPin &getSignalPin() const { return _signal; }
    inline const TerraDigitalPin &getChipEnablePin() const { return _chipEnable; }
    inline const TerraDigitalPin &getInterruptPin() const { return _interrupt; }
    inline uint8_t getChannelBits() const { return _channelBits; }
    inline uint8_t getSelectedChannel() const { return _channelSelect; }

protected:
    TerraPin _signal;                                       // Muxed signal pin (unused channel #)
    TerraDigitalPin _chipEnable;                            // Muxing chip enable pin (optional)
    TerraDigitalPin _interrupt;                             // Muxing interrupt pin (optional)
...
// Note: Virtual pins should always have a pin # >= tpin_virtual (100), and are separated
//       into groups of 16 per indexed expander (e.g. pins # 100-115 = expander[0], pins #
//       116-131 = expander[1], etc.).
class TerraPinExpander {
public:
    TerraPinExpander();
    TerraPinExpander(tposi_t expanderPos, uint8_t channelBits, IoAbstractionRef ioRef,
                     TerraDigitalPin interruptPin = TerraDigitalPin());
    ...
    inline tposi_t getExpanderPos() const { return _expander; }
    inline uint8_t getChannelBits() const { return _channelBits; }
    inline IoAbstractionRef getIoAbstraction() { return _ioRef; }
    inline const TerraDigitalPin &getInterruptPin() const { return _interrupt; }

protected:
    const tposi_t _expander;                                // Expander #/index (for virtual pin #)
    uint8_t _channelBits;                                   // Channel select bits (# of bus pins available)
    IoAbstractionRef _ioRef;                                // IoAbstraction instance
    TerraDigitalPin _interrupt;                             // Expander interrupt pin (optional)
    bool _usingISR;                                         // Using ISR flag
};
