...
#ifndef TerraInlines_HPP
#define TerraInlines_HPP
...
// Only have defines included at this point, complex inline imps at top of Terraduino.hpp

// Returns if pin is valid
inline bool isValidPin(pintype_t pin) { return pin != tpin_none; }
// Returns if channel is valid
inline bool isValidChannel(int8_t channel) { return channel != tpinchnl_none; }
...
inline bool isValidIndex(tposi_t index) { return index != tposi_none; }
// Returns if id key is valid
inline bool isValidKey(tkey_t key) { return key != tkey_none; }
// Returns if id type is valid
inline bool isValidType(tid_t type) { return type != tid_none; }
// Returns if frame is valid
inline bool isValidFrame(tframe_t frame) { return frame != tframe_none; }
...
inline Terra_UnitsType definedUnitsElse(Terra_UnitsType units1, Terra_UnitsType units2) {
    return units1 != Terra_UnitsType_Undefined ? units1 : units2;
}

// Returns the first unit in parameter list that isn't undefined, allowing defaulting chains to be nicely defined.
inline Terra_UnitsType definedUnitsElse(Terra_UnitsType units1, Terra_UnitsType units2, Terra_UnitsType units3) {
    return units1 != Terra_UnitsType_Undefined ? units1 : (units2 != Terra_UnitsType_Undefined ? units2 : units3);
}
...
struct I2CDeviceSetup {
    ...
    inline I2CDeviceSetup(TwoWire *i2cWire = TERRA_USE_WIRE, uint32_t i2cSpeed = 100000U, uint8_t i2cAddress = 0b000) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
    inline I2CDeviceSetup(uint32_t i2cSpeed, uint8_t i2cAddress = 0b000, TwoWire *i2cWire = TERRA_USE_WIRE) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
    inline I2CDeviceSetup(uint8_t i2cAddress, TwoWire *i2cWire = TERRA_USE_WIRE, uint32_t i2cSpeed = 100000U) : wire(i2cWire), speed(i2cSpeed), address(i2cAddress) { ; }
};
...
struct SPIDeviceSetup {
    ...
    inline SPIDeviceSetup(SPIClass *spiClass = TERRA_USE_SPI, uint32_t spiSpeed = F_SPD, pintype_t spiCS = -1) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
    inline SPIDeviceSetup(uint32_t spiSpeed, pintype_t spiCS = -1, SPIClass *spiClass = TERRA_USE_SPI) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
    inline SPIDeviceSetup(pintype_t spiCS, SPIClass *spiClass = TERRA_USE_SPI, uint32_t spiSpeed = F_SPD) : spi(spiClass), speed(spiSpeed), cs(spiCS) { ; }
};
...
struct UARTDeviceSetup {
    ...
    inline UARTDeviceSetup(SerialClass *serialClass = TERRA_USE_SERIAL1, uint32_t serialBaud = 9600U) : serial(serialClass), baud(serialBaud) { ; }
    inline UARTDeviceSetup(uint32_t serialBaud, SerialClass *serialClass = TERRA_USE_SERIAL1) : serial(serialClass), baud(serialBaud) { ; }
};
...
struct Twilight {
    ...
    inline Twilight() : sunrise(TERRA_NIGHT_FINISH_HR), sunset(TERRA_NIGHT_START_HR), isUTC(false) { ; }
